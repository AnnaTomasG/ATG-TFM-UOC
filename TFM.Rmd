...
res<-NbClust(hfNscaled, diss=NULL, distance = "euclidean", min.nc=2, max.nc=10, method = "ward.D2", index = "alllong")
...
res<-NbClust(hf, gower_dist, distance = NULL, min.nc=2, max.nc=10, method = "centroid", index = "all")
...
# Plot sihouette width (com més gran millor)
...
```
Ens surten 8 clusters com a millor opció
...
res.mc1 <- Mclust(hfNscaled, G = NULL)


res.mc2 <- Mclust(hf, G = NULL)
...
#variables numeriques escalades
...
summary(res.mc2)
plot(res.mc2, what = "density")
plot(res.mc2, what = "uncertainty")
...
hfNlog$creatinine_phosphokinase<-log(hfNlog$creatinine_phosphokinase)
hfNlog$serum_creatinine<-log(hfNlog$serum_sodium)
